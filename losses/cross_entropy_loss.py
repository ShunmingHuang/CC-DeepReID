...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # build the smoothed targets on the logits' own device, so this loss also
        # works on CPU-only setups (use_gpu stays a soft preference)
        device = inputs.device
        zeros = torch.zeros(log_probs.size(), device=device)
